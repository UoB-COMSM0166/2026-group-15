...
### How discussion shaped the game

Another recurring question in our meetings was how to make the game **meaningful as an environmental game for younger players**, rather than just another platformer with a theme attached. We often asked how to make the game both educational and enjoyable: how to design more interesting levels, suitable enemies and animals, clearer environmental tasks, and a more user‑friendly interface. These conversations directly shaped the structure of the game.

For example, we designed three themed worlds—**forest**, **underwater**, and **factory**—so that each level could present a different type of environmental issue. We also discussed what kinds of enemies and trapped animals would fit each setting, and how different pollutants should appear in gameplay. Common pollutants like plastic bottles are collected directly, while more hazardous elements such as acid pools require specific tools (e.g. limestone) to neutralise them. These ideas were developed collectively in meetings and then refined through implementation and playtesting, rather than being decided by a single person.

The **requirements workshop** also influenced how we worked. Once we had identified stakeholders and written epics, user stories, and acceptance criteria, it became much easier to connect development tasks to the wider purpose of the project. For instance, the story about collecting pollutants and rescuing animals affected not only the scoring system, but also level layout, tool design, and interface feedback. In this sense, the requirements process helped us move from a broad idea about an “environmental game” to a more focused and testable design.

### Challenges and how we adapted

Our collaboration was not without problems, and several issues forced us to adjust how we worked.

On the **art and asset side**, early tiles, tools, and UI elements were created without a shared standard for size and style. Once they appeared in the game, the inconsistencies became obvious: some tools were too large or too small, and assets drawn by different people did not always fit together visually. To address this, we agreed on common pixel sizes for tiles, characters, and items, revised older assets, and gradually narrowed final sprite production to a smaller number of members so that the art style became more consistent.

On the **movement and gameplay side**, early versions of jumping, underwater movement, and pipe traversal did not feel natural. Playtesters described the movement as unrealistic, and we also felt that the character sometimes seemed to hover rather than move with proper weight. After discussing this with other groups and revisiting our implementation, we realised that our first approach focused too much on directly changing positions rather than calculating movement through force, acceleration, and velocity. We later rebuilt these systems with that in mind, which made jumping, underwater buoyancy, and other environmental movement feel more believable.

We also ran into difficulties with the original **follower‑cat mechanic**. At first, we wanted the cat to physically follow the player through the level. However, once more hazards, vertical spaces, and movement changes were added, the mechanic became unstable. The cat could float unexpectedly, follow unnatural trajectories, or collide with acid and lava because of the way the follow logic worked. After several attempts to improve it, we decided to simplify the feature and move the cat into the UI as a progress indicator instead. This reduced the “pet companion” feeling, but it gave players a clearer sense of progression and avoided a large number of technical issues.

A further issue concerned **testing and evaluation**. At the beginning, we assumed that testing tasks would be shared naturally, but in practice this meant that recruitment, note‑taking, and data organisation were not always clearly owned by anyone. After the first evaluation round, we adjusted our process by making these responsibilities more explicit. This made later Think Aloud sessions, heuristic analysis, and SUS/NASA‑TLX data collection much more organised and easier to link back to concrete changes in the game.

### Reflection

Looking back, the most useful part of our teamwork was not that everything worked perfectly from the start, but that we were willing to adjust both the game and our way of working when something was clearly not effective. Asking what a change would mean for players and for the environmental message helped us keep the project aligned with its goals, while Agile‑style iteration helped us respond to technical and design problems without losing momentum. Overall, the project taught us not only how to build a three‑world platform game, but also how to balance creative ideas, technical limits, and player experience within a small team.
